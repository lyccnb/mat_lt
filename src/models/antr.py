...
class ANTR(nn.Module):
    def __init__(self, joiner, transformer, output_type, dim_feedforward, num_classes, num_queries,
                 num_decoder_embedding, aux_loss=True):
    ...
    def forward(self, samples, mask, targets, tgt_mask=None):
        ...
        nq = query_pos.size(0)
        prediction_times = torch.stack([t['prediction_duration'] for t in targets], axis=0)\
            .squeeze(1).repeat(1, nq, 1).permute(1, 2, 0)
        # print('query_pos.shape', query_pos.shape)
        # print('prediction_times.shape', prediction_times.shape)
        query_and_prediction_times = torch.cat([query_pos, prediction_times], axis=2)
        decoder_pos = self.query_time_embed(query_and_prediction_times.reshape(b * nq, self.hidden_dim + 1))\
            .reshape(b, nq, -1).permute(1, 0, 2)
...
class CriterionGreedyMatcher(nn.Module):
    ...
    def _get_tgt_permutation_idx(self, indices):
        ...
        batch_idx = torch.cat([torch.full_like(tgt, i) for i, (_, tgt) in enumerate(indices)], dim=0)
    ...
    def loss_labels(self, outputs, targets, indices, num_segments, log=True):
        ...
        # Create a tensor to represent all classes, and fill it with self.num_classes
        target_classes = torch.full(src_logits.shape[:2], self.num_classes,
                                    dtype=torch.int64, device=src_logits.device)

        # Update the target_classes tensor with the actual labels
    ...
    def loss_segments(self, outputs, targets, indices, num_segments):
        ...
        # Extract predicted segments and target segments
        ...
        # Calculate L1 regression loss
        loss_segment = F.l1_loss(src_segments, target_segments, reduction='sum') / num_segments

        # Calculate IoU loss
        loss_siou = 1 - torch.diag(segment_utils.generalized_segment_iou(src_segments, target_segments)).sum() / num_segments

        losses = {'loss_segment': loss_segment, 'loss_siou': loss_siou}
    ...
    def get_unrolled_timeline(self, outputs, targets):
        ...
        fps = targets[0]['fps']
        # 使用 torch.zeros_like 创建与输入张量相同设备和数据类型的零张量
        out_logits = torch.zeros_like(src_logits)[:, :int(torch.round(torch.max(torch.cat([t['prediction_duration'] for t in targets]))) + 1), :]
        for bidx in range(b):
            for sidx in range(len(src_segments_scaled[bidx])):
                ...
                # 使用 torch.where 替代嵌套循环
                mask = (torch.arange(out_logits.size(1)) >= s) & (torch.arange(out_logits.size(1)) < e)
                out_logits[bidx, mask, :] = torch.max(out_logits[bidx, mask, :], src_logits[bidx][sidx])
        # 使用 torch.zeros_like 创建与输入张量相同设备和数据类型的零张量
        ...
        target_classes = torch.zeros_like(out_logits[:, :, :c-1])
        for idx, t in enumerate(targets):
            target_classes[idx, :, :t['labels_onehot'].size(0)] = t['labels_onehot']
        # 释放中间变量
        del scale_factor, src_segments_scaled, out_logits
        return output_classes_onehot, target_classes

    @torch.no_grad()
    def loss_cardinality(self, outputs, targets, indices, num_segments):
        """Compute the cardinality error, i.e., the absolute error in the number of predicted non-empty segments
        ...
        # Calculate L1 loss for cardinality error
    ...
    def get_mAP(self, pred, labels, label_mask):
        ...
        # Clip the values in pred and labels to be within the range [0.0, 1.0]
    ...
    def get_accuracy(self, pred, labels, outputs, targets):
        ...
        for i in range(pred.shape[0]):
            # Use f-strings for creating the key
            k_r_p = f'acc_{int(targets[i]["ratio_idx"] * 100)}_{int(targets[i]["prediction_idx"] * 100)}'
            
            # Extract pred[i] and labels[i]
            pred_i = pred[i].detach().cpu()
            labels_i = labels[i].detach().cpu()
            
            # Create or append to the list associated with the key
            acc.setdefault(k_r_p, []).append(torch.cat((pred_i, labels_i), 1))
    ...
    def get_loss(self, loss, outputs, targets, indices, num_segments, **kwargs):
        ...
        assert loss in loss_functions, f'do you really want to compute {loss} loss?'
        
        # Call the appropriate loss function directly
        return loss_functions[loss](outputs, targets, indices, num_segments, **kwargs)
...
def build(args):
    ...
    criterion = CriterionGreedyMatcher(num_classes, matcher, weight_dict=weight_dict, eos_coef=args.eos_coef,
                                       losses=losses)
